...
Deno.serve(async (req) => {
  try {
    ...
    // SECURITY: Use a hardened app URL from env, never from request headers.
    // Set APP_BASE_URL in environment variables (e.g. https://app.base44.com/yourapp)
    const appBaseUrl = Deno.env.get('APP_BASE_URL') || 'https://app.base44.com';
    const portalUrl = `${appBaseUrl}/employer-portal?proposal=${proposal_id}`;
...
<body style="font-family: sans-serif; color: #1a1a2e; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p style="font-size: 12px; color: #94a3b8; border-top: 1px solid #e2e8f0; padding-top: 16px; margin-top: 24px;">
    ...
    // SECURITY: Do not expose internal error details to the caller
    console.error('[sendProposalEmail] error:', error.message);
    return Response.json({ error: 'Failed to send proposal email' }, { status: 500 });
  }
});
