...
export default function ProposalCard({ proposal, onView, onEdit, onReject }) {
  ...
  const handleSend = (e) => {
    e.stopPropagation();
    updateStatus.mutate({ status: "sent", sent_at: new Date().toISOString() });
  };
        ...
        <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {proposal.plan_summary?.length > 0 && (
                <div className="flex items-center gap-1 mt-2 flex-wrap">
                  {proposal.plan_summary.slice(0, 3).map((plan, i) => (
                    <Badge key={i} variant="outline" className="text-[10px]">
                      {plan.plan_name || plan.name}
                    </Badge>
                  ))}
                  {proposal.plan_summary.length > 3 && (
                    <span className="text-xs text-muted-foreground">+{proposal.plan_summary.length - 3} more</span>
                  )}
                </div>
              )}
                ...
                <div className="flex items-center gap-2 mt-2">
                  <Link to={`/cases/${proposal.case_id}`} onClick={e => e.stopPropagation()}>
                    <Button variant="outline" size="sm" className="h-6 text-[10px] gap-1 text-green-700 border-green-200 bg-green-50 hover:bg-green-100">
                      <ChevronRight className="w-3 h-3" /> Open Enrollment
                    </Button>
                  </Link>
                </div>
              )}
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0" onClick={e => e.stopPropagation()}>
            ...
            {proposal.status === "draft" && (
              <Button size="sm" className="h-7 text-xs gap-1" onClick={handleSend} disabled={updateStatus.isPending}>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-44">
                ...
                <DropdownMenuItem onClick={() => cloneProposal.mutate()}>
                  <Copy className="w-3.5 h-3.5 mr-2" /> Clone (New Version)
                </DropdownMenuItem>
                {["sent","viewed"].includes(proposal.status) && (
                ...
                <DropdownMenuItem onClick={() => deleteProposal.mutate()} className="text-destructive">
