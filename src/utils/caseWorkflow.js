...
export function getNextStageLabel(stage) {
  ...
  return nextStage ? STAGE_LABELS[nextStage] || nextStage.replace(/_/g, " ") : null;
}

export function getCaseNextStep(caseData, meta = {}) {
  switch (caseData?.stage) {
    case "draft":
      return "Start census collection";
    case "census_in_progress":
      return "Validate the census";
    case "census_validated":
      return "Prepare the quote request";
    case "ready_for_quote":
      return "Launch quoting";
    case "quoting":
      return meta.quoteCount > 0 ? "Review quote scenarios" : "Create the first quote scenario";
    case "proposal_ready":
      return meta.proposalCount > 0 ? "Send the proposal" : "Build the proposal";
    case "employer_review":
      return "Follow up with the employer";
    case "approved_for_enrollment":
      return meta.enrollmentCount > 0 ? "Invite employees" : "Create the enrollment window";
    case "enrollment_open":
      return "Track elections and reminders";
    case "enrollment_complete":
      return "Finalize enrollment";
    case "install_in_progress":
      return "Complete carrier install";
    case "renewal_pending":
      return "Prepare the renewal strategy";
    case "active":
      return "Monitor tasks and renewal timing";
    default:
      return "Review the case details";
  }
}
...
export function getCaseBlocker(caseData, meta = {}) {
  ...
  if (meta.exceptionCount > 0) {
    return `${meta.exceptionCount} open exception${meta.exceptionCount === 1 ? "" : "s"}`;
  }
  if (meta.openTaskCount > 0) {
    return `${meta.openTaskCount} open task${meta.openTaskCount === 1 ? "" : "s"}`;
  }
  return null;
}