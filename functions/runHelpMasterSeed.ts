...
// [target_code, page_code, section_code, type, name, label, component_key, field_name,
//  control_name, action_code, workflow_code, grid_code, grid_column_code, path, keywords, role_scope, sort, help_enabled, required_coverage]
const INVENTORY = [
...
const TEMPLATES = {
  field:       { applies_to: ["field","toggle","radio_option","select_option"],
    help_title: "{target_label}",
    short_help_text: "This field is used to capture or display {target_label}.",
  ...
  button:      { applies_to: ["button","action","link"],
    help_title: "{target_label}",
    short_help_text: "This action allows the user to {target_label}.",
  ...
  grid_column: { applies_to: ["grid_column"],
    help_title: "{target_label}",
    short_help_text: "This column shows {target_label} for each record in the grid.",
...
function findTemplate(targetType) {
  ...
  return TEMPLATES.field; // fallback
}
...
function generateContent(item) {
  ...
  return {
    ...
    version_no: 1,
    language_code: "en",
    help_title: t(tpl.help_title),
    short_help_text: t(tpl.short_help_text),
    ...
    allowed_values_text: t(tpl.allowed_values_text),
    examples_text: t(tpl.examples_text),
    dependency_notes_text: t(tpl.dependency_notes_text),
    warnings_text: t(tpl.warnings_text),
    validation_notes_text: t(tpl.validation_notes_text),
    related_topics_text: t(tpl.related_topics_text),
    search_keywords: t(tpl.search_keywords),
...
// ─── UPSERT HELPER: create if not exists by business key ─────────────────────

async function upsertByCode(entitySDK, existingMap, codeKey, record) {
  const code = record[codeKey];
  const existing = existingMap[code];
  if (existing) return existing; // idempotent skip
  const created = await entitySDK.create(record);
  existingMap[code] = created;
  return created;
}

async function bulkUpsertByCode(entitySDK, codeKey, records) {
  const existing = await entitySDK.list("-created_date", 1000);
  ...
  const all = await entitySDK.list("-created_date", 1000);
...
Deno.serve(async (req) => {
  ...
  // ── 1. MODULES ────────────────────────────────────────────────────────────
  const moduleByCode = await bulkUpsertByCode(db.HelpModule, "module_code", MODULES);
  stats.modules = Object.keys(moduleByCode).length;

  // ── 2. PAGES ──────────────────────────────────────────────────────────────
  const pageModuleCodeByPageCode = Object.fromEntries(PAGES.map(p => [p.page_code, p.module_code]));
  const pagesWithIds = PAGES.map(p => ({
    ...p, is_system_defined: true,
    help_module_id: moduleByCode[p.module_code]?.id || null,
  }));
  const pageByCode = await bulkUpsertByCode(db.HelpPage, "page_code", pagesWithIds);
  stats.pages = Object.keys(pageByCode).length;

  // ── 3. SECTIONS ───────────────────────────────────────────────────────────
  const sectionsWithIds = SECTIONS.map(s => ({
    ...s, is_system_defined: true,
    help_page_id: pageByCode[s.page_code]?.id || null,
    parent_section_id: null, // resolved in second pass if needed
  }));
  const sectionByCode = await bulkUpsertByCode(db.HelpSection, "section_code", sectionsWithIds);
  stats.sections = Object.keys(sectionByCode).length;

  // ── 4. TARGETS from inventory ─────────────────────────────────────────────
  ...
  const targetByCode = await bulkUpsertByCode(db.HelpTarget, "target_code", targetRecords);
  stats.targets = Object.keys(targetByCode).length;

  // ── 5. MANUAL TOPICS ──────────────────────────────────────────────────────
  const topicsWithIds = MANUAL_TOPICS.map(t => ({
    ...t,
    help_module_id: moduleByCode[t.module_code]?.id || null,
    published_at: t.is_published ? now : null,
  }));
  const topicByCode = await bulkUpsertByCode(db.HelpManualTopic, "topic_code", topicsWithIds);
  stats.manual_topics = Object.keys(topicByCode).length;
  ...
  const newMaps = [];
  for (const m of TOPIC_TARGET_MAPS) {
    const topic = topicByCode[m.topic_code];
    const target = targetByCode[m.target_code];
    if (!topic || !target) continue;
    const key = `${topic.id}__${target.id}`;
    if (mapKeys.has(key)) continue;
    newMaps.push({ help_manual_topic_id: topic.id, topic_code: m.topic_code, help_target_id: target.id, target_code: m.target_code });
  }
  if (newMaps.length) {
    try { await db.HelpManualTopicTargetMap.bulkCreate(newMaps); } catch (_) {}
  }
  stats.topic_maps_new = newMaps.length;

  // ── 7. GENERATE HELP CONTENTS (upsert by help_target_code) ───────────────
  const existingContents = await db.HelpContent.list("-created_date", 2000);
  const existingContentByTargetCode = Object.fromEntries(existingContents.map(c => [c.help_target_code, c]));

  const newContents = [];
  const allInventoryItems = INVENTORY.map(row => {
    const [target_code, page_code, , target_type, target_name, target_label, , , , , , , , , search_keywords, role_scope] = row;
    return { target_code, page_code, target_type, target_name, target_label, search_keywords, role_scope };
  });

  for (const item of allInventoryItems) {
    if (existingContentByTargetCode[item.target_code]) continue;
    const target = targetByCode[item.target_code];
    if (!target) continue;
    const generated = generateContent(item);
    const pg = pageByCode[item.page_code];
    newContents.push({
      help_target_id: target.id,
      help_target_code: item.target_code,
      module_code: pg?.module_code || null,
      page_code: item.page_code,
      role_visibility: Array.isArray(item.role_scope) ? item.role_scope.join(", ") : (item.role_scope || "all"),
      ...generated,
  ...
  for (let i = 0; i < newContents.length; i += BATCH) {
    await db.HelpContent.bulkCreate(newContents.slice(i, i + BATCH));
  }
  stats.contents_new = newContents.length;

  // ── 8. HELP CONTENT VERSIONS (snapshot for newly created content) ─────────
  ...
  const versionedContentIds = new Set(existingVersions.map(v => v.help_content_id));

  const newVersions = [];
  for (const c of allContents) {
    if (versionedContentIds.has(c.id)) continue;
    newVersions.push({
      help_content_id: c.id,
      help_target_code: c.help_target_code,
      version_no: 1,
      change_type: "create",
      snapshot_payload: {
        help_title: c.help_title, short_help_text: c.short_help_text,
        detailed_help_text: c.detailed_help_text, content_status: c.content_status,
        content_source_type: c.content_source_type,
      },
      change_summary: "Initial system-generated help content",
      changed_by: null,
    });
  }
  ...
  stats.versions_new = newVersions.length;

  // ── 9. AI TRAINING QUEUE ──────────────────────────────────────────────────
  const existingQueue = await db.HelpAITrainingQueue.list("-created_date", 2000);
  const queuedEntityIds = new Set(existingQueue.map(q => q.source_entity_id));

  const queueItems = [];
  for (const c of allContents) {
    if (queuedEntityIds.has(c.id)) continue;
    queueItems.push({ source_entity_type: "HelpContent", source_entity_id: c.id, source_target_code: c.help_target_code, change_reason: "initial seed", queue_status: "queued", attempt_count: 0, queued_at: now });
  }
  for (const t of Object.values(topicByCode)) {
    if (!t.is_published) continue;
    if (queuedEntityIds.has(t.id)) continue;
    queueItems.push({ source_entity_type: "HelpManualTopic", source_entity_id: t.id, source_target_code: null, change_reason: "initial seed", queue_status: "queued", attempt_count: 0, queued_at: now });
  }
  for (let i = 0; i < queueItems.length; i += BATCH) {
    await db.HelpAITrainingQueue.bulkCreate(queueItems.slice(i, i + BATCH));
  }
  stats.ai_queue_new = queueItems.length;

  return Response.json({ success: true, stats, message: "Help master seed completed successfully" });
});